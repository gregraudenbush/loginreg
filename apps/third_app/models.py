...
class UserManager(models.Manager):
    ...
    def val(self, post):
        ...
        if len(post['email']) < 1 or len(post['first_name']) < 1 or len(post['last_name']) < 1:
        ...
        if not errors:
            if User.objects.filter(email = post['email']).exists():
                ...
                user = User.objects.create(first_name = post['first_name'], last_name = post['last_name'], email=post['email'],password=hashed_pw)
